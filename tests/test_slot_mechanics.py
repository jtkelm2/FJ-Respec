...
import pytest
from core.type import Slot, Card, CardType, SlotKind
...
class TestSlotBookkeeping:
    ...
    def test_slotting_into_new_slot_auto_deslots_from_old(self):
        s1 = Slot("t", SlotKind.HAND)
        s2 = Slot("t", SlotKind.HAND)
        c = _card()
        s1.slot(c)
        s2.slot(c)
...
class TestSlotEdgeCases:
    ...
    def test_empty_slot_is_empty(self):
        assert Slot("t", SlotKind.HAND).is_empty()

    def test_non_empty_slot_is_not_empty(self):
        assert not Slot("t", SlotKind.HAND, cards=[_card()]).is_empty()

    def test_draw_last_card_makes_slot_empty(self):
        s = Slot("t", SlotKind.HAND, cards=[_card()])
        s.draw()
        assert s.is_empty()

    def test_constructor_slots_all_cards(self):
        cards = [_card("a"), _card("b")]
        s = Slot("t", SlotKind.HAND, cards=cards)
    ...
    def test_deslot_absent_card_raises(self):
        with pytest.raises(AssertionError):
            Slot("t", SlotKind.HAND).deslot(_card())

    def test_draw_from_empty_raises(self):
        with pytest.raises(AssertionError):
            Slot("t", SlotKind.HAND).draw()
